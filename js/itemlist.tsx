...
const ItemRow = React.createClass({
  ...
  suppField: function (name) {
    return extVal(
      ...
      extractLookup(this.props.data.supplementalFields, name)
    );
  },
  render: function () {
    const linkHTML = this.props.data.coreLinks ? (
      <span>
    ...
    return (
      <tr className={css(styles.itemRow)} onClick={this.props.clickHandler}>
        <td className={css(styles.itemField)}>{this.props.data.name}</td>
        <td className={css(styles.itemField)}>{this.suppField("Date")}</td>
